...
def get_movies_by_year(release_date):
    with get_db_cursor() as cursor:
        cursor.execute("""select * from movie where release_date=%s;""",[release_date])
        return cursor.fetchall()
...
def create_ticket(booking_id, seat_id):
    with get_db_cursor() as cursor:
        cursor.execute("""insert into ticket (booking_id, seat_id) values (%s,%s) returning id;""", [booking_id, seat_id])
        return cursor.fetchone()

def create_room(cinema_id, number):
    with get_db_cursor() as cursor:
        cursor.execute("""insert into room (cinema_id, number) values (%s,%s) returning id;""", [cinema_id, number])
        return cursor.fetchone()
